...
func (c *Controller) Reconcile(ctx context.Context, nodeClaim *karpv1.NodeClaim) (reconcile.Result, error) {
	...
	if err := c.tagInstance(ctx, nodeClaim, nodeClaim.Status.ProviderID); err != nil {
...
func (c *Controller) tagInstance(ctx context.Context, nc *karpv1.NodeClaim, id string) error {
	...
	err := c.kubeClient.Get(ctx, types.NamespacedName{Name: nc.Spec.NodeClassRef.Name}, nodeclass)
	if err != nil {
		return err
	}
	tags := nodeclass.Spec.FreeFormTags
	// Remove tags which have been already populated
	getIns, err := c.instanceProvider.Get(ctx, id)
	if err != nil {
		return fmt.Errorf("tagging nodeclaim, %w", err)
	}
	tags = lo.OmitByKeys(tags, lo.Keys(getIns.FreeformTags))
	if len(tags) == 0 {
		return nil
	}
